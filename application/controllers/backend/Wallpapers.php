...
class Wallpapers extends BE_Controller {
	...
	function save( $id = false ) {
		...
		// if id is false, this is adding new record
			...
			if($data['is_wallpaper'] == 1 || $data['is_gif'] == 1){
				...
				$typelayout = $this->input->post('image_type_por_hor');
				if ( ! $this->insert_images( $this->input->post('images1'), 'wallpaper', $data['wallpaper_id'] ,$typelayout)) {
	 ...
	 */
	function delete($wallpaper_id = 0){
			// start the transaction
			$this->db->trans_start();
